...
# Въведение в обработката на данни с Raku
...
Една от настоящите ми житейски мисии е да ускоря идването на следващата [Зима на ИИ](https://en.wikipedia.org/wiki/AI_winter).
Решил съм да използвам Раку (Raku), за изпълването на тази мисия. (Поне в началото...)

Освен това намирам термина "изкуствен интелект" (ИИ) за брилянтна маркетингова фраза за извличане на пари от
военния комплекс на САЩ и всякакви финансови инвеститори. Когато хора, които твърдят, че са, да речем, "професионалисти в областта на науката за данните"
използват тази фраза в професионална среда, аз ставам силно подозрителен към техния "професионализъм".
(Това означава, че най-вероятно са някакви набедени подражатели.)

Но нека да се върнем към ускоряването на следващота зима на ИИ. Ето го моят план:

1. Научаването на много хора [как да бъдат самозвани учени в областта на данните](https://github.com/antononcube/HowToBeADataScientistImpostor-book)

2. Изработване на [програмни инструменти](https://conf.raku.org/talk/157), които да улеснят баристите на ИИ-кодове

3. Изчакване на достатъчно много ИИ-хора да се ударят в достатъчно много стени на ИИ.
...
без талантен от контингент от практикуващи ИИ".

Печалбата в точка 5 идва от това, че "полето" е "изчистено", следователно по-малка конкуренция за получаване на финансиране и инвестиции.
(Дори след това уточнение някои читатели все още могат да смятат, че горният ми списък е твърде сходен с този, измислен от гномите в
"Гномите" в [South Park's "Gnomes"](https://en.wikipedia.org/wiki/Gnomes_(South_Park));
и това не ме притеснява.)

Повечето "даннови учени" (data scientists), прекарват по-голямата част от времето си в събирането и обработката на данни.
Не в данново анализаране, или изкуствения интелект, или каквато и да е там "наистина научна" работа...
Така че, ако сериозно искам да повлияя на кривите на развитие на ИИ, тогава трябва да се заема сериозно с повлияването
на формолировките на заклинания за събиране на данни и работа с данни на различни езици за програмиране; [AAv2].
Тъй като твърдо вярвам, че е добре от време на време да ядеш храната на собственото си куче,
аз програмирах (през последните няколко седмици) пакети за обработка на данни на езика Raku.
Кой знае, това може да е начин да ракунизираме на изкуствения интелект и --
[да перифразирам Лари Уол](https://en.wikipedia.org/wiki/Raku_(programming_language)#History)
-- някои потребители могат да получат своята поправителна доза. (Вж. също [FB1].)
...
[AAr1, AAv6],
който има разговорен агент за генериране код чрез Raku пакети, [AAp7].
За да обсъдим и да дадем примери за работа с данни, трябва да използваме определени функционалности за добиване на данни.
Поради тази причина, по-долу са дадени обяснения и примери за използване на Raku пакети за извличане на популярни,
добре познати таблици от данни, и за генериране на случайни таблици от данни.

Този документ е доста технически - читателите могат просто да прочетат или да прескочат следващия раздел и раздела
"Да го правим като Кро" и да приключат. Някои може да искат да погледнат и да прескочат супертехническата версия
"Data wrangling with Raku", [AA1].


**Забележка:** Понякога кодът по-долу може да има израза на Raku `==>encode-to-wl()`.
Това е за сериализиране на Raku обекти като
...
**Забележка:** Целевата аудитория на този документ се състои предимно от хора, запознати с културите и култовете на Перл (Perl) и Раку (Raku).
...
**Забележка:** Както може да се види в (дългия) видео-запис на презентацията
["Многоезичен разговорен агент за събиране на данни (разширена версия)"](https://www.youtube.com/watch?v=KlEl2b8oxb8),
[AAv6], на един писан на Raku разговорен агент за добив на данни ще му са му значително от полза възможности на Raku за обработка на данни.

**Забележка:** Изреченията с "универсални", проверими или възпроизводими твърдения, твърдения и кодове използват формата "ние".
Изявленията с лични мнения и решения на автора използват формата "аз".
Алтернативно, просто използвах каквато форма ми се стори по-лесна или по-естествена за писане.
...
... *или* ***"Раку-за" срещу "с-Раку "*** *(също така, може би, "удоволствие за" срещу "с удоволствие".)*
...
- Работа с данни ***с*** Raku означава използване на структурите от данни и езика за програмиране на Raku за работа с данни.
...
Следва пример за обработка на данни ***с*** Raku:
...
- Представя усилията "Raku-за" с шестоъгълника
- Представлява усилията "с Raku" в долния десен ъгъл
...
Също така, разбира се, превод от Raku към други програмни езици.

**Забележка:** Дълго време използвах принципа "дрехите нямат император", [AAv1, CARH1], който,
разбира се, принадлежи към подхода "Raku-за-предсказвания". Чрез снабдяването на Raku с (i) възможности за обработка на данни и
(ii) възможност за генериране на код на Raku за обработка на данни, бих казал, че тези дрехи могат да бъдат подходящо облечени.
...
Нека дефинираме или очертаем основните понятия на нашето начинание за боравене с данни в Raku.
...
Системата Mathematica използва даннови масиви. S, R и [pandas](https://pandas.pydata.org) от Python използват даннови рамки.

Системата Raku, представена в този документ, използва даннови масиви. Ето пример за набор от данни с 3 реда и 2 колони:
...
Ето как би била структурирана съответната даннова рамка:
...
Не искам да създавам специален тип (клас) за даннови масиви или даннови рамки - искам да използвам стандартните структури от данни на Raku.
(Поне на този етап от работата ми с данни в Raku.)

Причините за това са:

1. Данните могат да се взимат и трансформират с типични, вградени команди.

    - Тоест, без придържане към определена методология за преобразуване на данни или специализирани пакети.

2. Използването на стандартни, вградени структури е вид решение за "потребителски интерфейс".

3. "Потребителското преживяване" може да бъде постигнато с- или осигурено от други парадигми и пакети за преобразуване.
...
Редът в по-горният списък показва важността по време на имплементирането съответните функционалности:

- Функционалностите за първите две са основни и имат unit тестове ("unit tests")

- Освен това ние се съобразяваме с използването на последните две.

Когато рамката и съзвездието от функционалности за обработка на данни узреят, всичките четири структури от данни ще имат
правилна и последователна обработка.

### Целеви потребители

Целевите потребители са учени в областта на данните (на пълно работно време, на непълно работно време и напълно начинаещи), които искат да:

- да обработват данни от типични набори от данни за науката за данните с Raku
...
### Разглеждано работни процеси

Следващата блок-схема обхваща работните процеси за преобразуване на данни, които разглеждаме:
...
***Модел на работните потоци за преобразуване на данни*** (DTWM).
В настоящия документ разглеждаме тази методология и блок-схемата като синоними.

Ето някои свойства на методологията/поточната диаграма:

- Блок-схемата е за таблични набори от данни или за списъци (масиви) или речници (хешове) на таблични набори от данни

- В блок-схемата само зареждането на данните и обобщеният анализ не са по избор

- Всички други стъпки не са задължителни

- Трансформации като вътрешни обединения са представени чрез блока "Комбиниране на групи".

- Предполага се, че в реалните приложения трябва да се извършат няколко итерации (цикли) върху блок-схемата

В света на езика за програмиране R оранжевите блокове представляват така наречения модел Split-Transform-Combine (Разделяне-Трансформиране-Съединяване);
вижте статията
["Стратегия Split-Apply-Combine за анализ на данни"] (https://www.jstatsoft.org/article/view/v040i01)
от Хадли Уикам (Hadley Wickham), [[HW1](https://www.jstatsoft.org/article/view/v040i01)].

**Забележка:** R беше (и вероятно все още е) доста загадъчен език за програмиране, така че изричното въвеждане на
Разделяне-Трансформиране-Комбиниране беше от голяма полза за програмистите на R. От друга страна, този модел е доста стар
и добре познат: той е присъщ на SQL и се среща в паралелното програмиране.
(Например, вижте функцията WL
[`ParallelCombine`](http://reference.wolfram.com/mathematica/ref/ParallelCombine.html),
[WRI2].)

Представяме ви прост сценарий за проиграване:

1. Получаване на табличен набор от данни от склад.
...
Горният списък от стъпки е само един от възможните работни процеси. За по-подробни примери и обяснения вижте [AA1, AAv2, AAv3, AAv4].

### Основни операции

Тези операции са само основни:

- Избор на колона, преименуване и изтриване

- Избор и изтриване на редове

- Транспониране

- Вътрешни, леви и десни обединения

- Групиране по критерии

(Транспонирането на таблични или пълномащабни данни също е основна операция на функционалното програмиране.)

При обработката на данни и анализа на данни следните три операции не са основни, но все пак са фундаментални:

- [Кръстосана табулация](https://en.wikipedia.org/wiki/Contingency_table)

- [Преобразуване на дълъг формат](https://en.wikipedia.org/wiki/Wide_and_narrow_data#Narrow)

- [Преобразуване на широк формат](https://en.wikipedia.org/wiki/Wide_and_narrow_data#Wide)

За повече подробности вижте [AA1, Wk1, Wk2, AAv1-AAv4].

**Забележка:** Пакетът
["Data::Reshapers"](https://github.com/antononcube/Raku-Data-Reshapers),
[AAp2], предоставя всички функции, споменати в този подраздел.
...
... *или* ***"Придобиване на данни от добре познати набори от данни в Raku "***.

Трябва да имаме достъп до някои типични набори от данни, използвани в часовете по преподаване на статистика или в книги и пакети, които
илюстрират концепции от областта на статистиката или обясняват свързаните с тях софтуерни проекти и ноу-хау.
Също така, разбира се, притежаването на тези набори от данни би било от голяма полза за самозваните учени в областта на данните и баристите на кодове в
взаимодействието им с другите.

Пакетът Raku
["Data::ExamplesDatasets"](https://github.com/antononcube/Raku-Data-ExampleDatasets),
[AAp3], предоставя функции за получаване на (сравнително добре познати) набори от примерни данни.
Самият пакет съдържа само метаданни за наборите от данни - наборите от данни се изтеглят от хранилището
["Rdatasets"](https://github.com/vincentarelbundock/Rdatasets/),
[VAB1].

Тук получаваме известна примерна съвкупност от данни с помощта на regex:
...
Ето размерите на току-що импортирания даннов масив:
...
Можем да получим URL адреса на документацията за този набор от данни, като използваме функцията `item-to-doc-url`:
...
### Обобщаване на колекцията от набори от данни

Ето обобщение на набора от данни с метаданни без колоната със заглавия и колоните с 
URL адреси на данни и документация:
...
Ето хистограма на разпределението на броя на редовете в наборите от данни с примери
(получаване на данните в Raku, изчертаване на хистограмата с WL):
...
### Идентификатори на набори от данни

Идентификаторът на даннов масив се състои от име на пакет и име на елемент, разделени с `"::"`.
Както може да се види в обобщената таблица по-горе, един пакет може да има няколко елемента и едно и също име на елемент може да бъде намерено
в няколко пакета. Следователно при някои спецификации на набори от данни функцията `example-dataset` дава предупреждение
за множество пакети, без да извлича данни:
...
Тук извличаме конкретен набор от данни, като използваме идентификатор, който се състои от името на пакета и името на елемента.
(разделени с "::"):
...
Ако това наречие е дадено, `example-dataset` съхранява данните, извлечени от уеб, в директорията `XDG_DATA_HOME`
и впоследствие ги извлича оттам. Вижте
["Freedesktop.org Specifications"](https://specifications.freedesktop.org)
и [JS1] за повече подробности относно това каква е конкретната стойност на променливата на средата `XDG_DATA_HOME`.
...
Вместо примерни набори от данни и справяне с потенциални проблеми, като например извличането им или просто намирането на един или два,
или пет, които отговарят на това, с което искаме да експериментираме, защо просто не генерираме случайни таблични набори от данни?!

Функцията `random-tabular-dataset` от пакета
["Data::Generators"](https://modules.raku.org/dist/Data::Generators:cpan:ANTONOV),
[AAp4], генерира случайни таблични набори от данни, като използва като аргументи спецификациите shape- и generators.
...
Можем също така да генерираме случайни набори от данни, като посочим имена на колоните и 
...
**Забележка:** Колоната "История" няма зададен от потребителя генератор, 
затова за нея беше избран генератор (по случаен начин).
...
автоматично съответните функции за вземане на проби. Тук генерираме случаен набор от таблични данни с $10$ редове,
колони "Eva", "Jerry" и "Project", като на всяка колона се задават стойности от малък набор от стойности:
...
Тогава добрите баристи на код могат да модифицират този код според изискванията на клиента.

Тук зареждаме цялостния пакет за превод [AAp8]:
...
Тук дефинираме команда-низ, който задава работен процес за обработка на данни:
...
**Забележка:** От една и съща команда на естествен език можем да генерираме код за обработка на данни на други езици:
Julia, Python, R, Wolfram Language.

Преводачът на команди от естествен език за обработка на данни се основава на DTWM, описан в раздела
"Datum fundamentum" по-горе. За по-обширни примери за използването му вижте презентацията
...
... *или* ***"Използване на уеб услуга, създадена от Cro, за генериране на код за обработка на данни"***.

Мислейки по-нататък за професионалния живот на самозваните даннови учени и баристите на кодове, 
можем да предоставим уеб услуга, която превежда DSL на естествен език в изпълним код. 
Реализирах такава уеб услуга чрез съзвездието от Raku библиотеките Cro; 
по-долу ще я наричаме Cro Web Service (CWS). Вижте видеото [AAv5] за демонстрация.

### Получаване на код чрез Web API

#### Разработване на данни от кода на Raku

Ето един пример за използване на CWS чрез функцията за уеб взаимодействие на Mathematica
...
CWS предоставя код за други езици за програмиране и видове работни процеси.
Ето един пример с код на работен процес за латентен семантичен анализ (LSA) в R:
...
което позволява допълнителна персонализация на резултатите от анализа и тълкуването.
...
2. Използвайки функцията за преобразуване на реч в текст на Siri, въведете текст

3. Shortcuts извиква CWS

4. Резултатът се връща във вид на JSON към Shortcuts

5. Shortcuts разглежда резултата от CWS

6. Ако парсирането е успешно

    1. Shortcuts издава известие

    2. Извършва съответният код е наличен в клипборда
...
[AAv5], демонстрира горните стъпки чрез диктовка на командите на английски език.)
...
Принципът "има повече от един начин да се направи едно нещо" често се оказва твърде ограничаващ или твърде блокиращ.
Според моя опит баристите на кодове и мениджърите на ИТ технологии предпочитат един начин за правене на нещата.
Също така, да не се излагат прекалено много на Парадокса на избора.
И изобщо, предпочитат се доброволното опростяване и предвидимата посредственост.
Което е добре, тъй като ние имаме решение, което добре служи на мало-акълните, когато са праволинейни.

Ето елементите на предложеното решение:

- Български (или английски) език, специфичен за проблемната област (DSL)

- Преводачи на DSL за най-популярните езици за обработка на данни

- Лесна за научаване и съобразена с нуждите методология за обработка на данни

- Генерираните програмни кодове се очаква да бъдат "добри отправни точки"

    - Т.е. да бъдат допълнително нагласени от потребителите в съответствие с желаните резултати.

Можем да перифразираме и обобщим горното по следния начин:

- Бързото специфициране на работните процеси за обработка на данни се постига чрез използване на абстрактно представяне 
  на граматика на командите на естествения език за обработка на данни, което позволява различни
  да се създават реализации на операции по обработка на данни за всеки език за програмиране, който представлява интерес.

Нека също така да посочим, че предложеното DSL решение за обработка на данни не обхваща *всички възможни*
на данни, но твърдя, че за колекции от таблични данни можем да рационализираме всички сложни
обработка на данни до голяма степен.

Ако трябва да се изразя отново, предполагам, че 60÷80 % от вашите работни процеси по обработка на данни могат да бъдат обработени с това решение.
(Да, YMMV и принципът на Парето в комбинация.)

### [Позоваване към авторитети](https://en.wikipedia.org/wiki/Argument_from_authority)

Разбира се, добре е да обосновем подходът, като се посочим как той съвпада с твърденията на определени авторитетни
фигури. Да речем, Лари Уол или Лао Дзъ.

### Лари Уол

В интервюто на Лари Уол, поместено в книгата "Masterminds of programming", [FB1], Лари описва Perl 6 (Raku) като:
...
Решението за работа с данни чрез DSL се придържа към твърдението на Лари:
...
- Скриването на тези измерения се постига чрез използване на спецификации на DSL на естествен език, които генерират изпълним код
  за тези измерения

- Дизайнът на граматиката и действията, присъщ на Raku, осигурява възможността за скриване на измеренията

### Лао Дзъ

Подходът може да бъде допълнително обоснован чрез позоваване на Лао Дзъ
["Tao Te Ching"] (https://en.wikipedia.org/wiki/Tao_Te_Ching),
[Книга 1, глава 11](https://www.egreenway.com/taoism/ttclz11.htm).

Ето превод на тази глава:

> Тридесетте спици се обединяват в един кораб;   
но от празното място за оста зависи използването на колелото.  
Глината се превръща в съдове;    
но употребата им зависи от празната им кухина.  
Вратата и прозорците се изрязват от стените, за да образуват апартамент;    
но употребата му зависи от празното пространство вътре.  
Следователно онова, което има положително съществуване, служи за изгодно приспособяване,   
а онова, което няма такова, за действителна полезност.  
> ~ Превод: Джеймс Ледж, 1891 г., глава 11

Ето някои моменти, които изясняват как решението за обработка на данни в DSL може да се разглежда като проява на
изложения принцип:
...
- DSL на естествен език за обработка на данни позволява бързо специфициране на работните процеси за обработка на данни
  на различни езици за програмиране, защото:
    ...
    - Естественият език крие много семантични и синтактични детайли на езика за програмиране.

- "Отрицателното съществуване" ("празнотата") се осигурява от използването на естествения език:
  не синтаксис на езиците за програмиране, арканичност и неестественост.

- "Тридесетте спици на колелото" са представени от множеството езици за програмиране и библиотеки
  на които се превежда DSL.

**Забележка:** Подобна обосновка използвах и за превода на изрази от Mathematica за High Performance Fortran (HPF).   
Вижте [AA3, AAn3]. По принцип "отрицателното съществуване" на изразите за функционално програмиране в Mathematica и
HPF са донякъде сходни и това позволи да се напише транслатор от Mathematica към HPF.
...
Имах намерение да публикувам този документ в ["Raku Advent Calendar"](https://raku-advent.blog).
(Не се случи.)

Един от
["Raku Advent Calendar"](https://raku-advent.blog)
организаторите, след като видя един от моите съвсем първоначални чернови, ме помоли да направя нещо "по-леко".
(В сърдечен смисъл.)
...
-------
## Приказка за вълка, овена и енота

... *или* ***"Свързване на Raku с Mathematica"***.

Кратката версия на приказката е следната:

> Вълкът, овенът и енотът разговаряли през едно тунелче. Тунелчето и било предоставено от [ZeroMQ](https://zeromq.org). (Край.)

Дългата версия е дадена в
["Свързване на Раку с Mathematica"](https://github.com/antononcube/RakuForPrediction-book/blob/main/Articles/Connecting-Mathematica-and-Raku.md),
[AA2].
Използване на [ZeroMQ](https://zeromq.org) и
[Raku-to-WL serializer](https://github.com/antononcube/Raku-Mathematica-Serializer),
[AAp6], можем да изпълняваме команди на Raku в тетрадки на Mathematica.

Защо това е полезно? Mathematica е най-мощната математическа софтуерна система и има една от най-старите,
най-зрелите решения за тетрадки. Следователно свързването на Raku с Mathematica позволява улесняване и използване на
някои интересни синергии между двете.

Ето и някои ориентирани към Raku последици от последното твърдение:

- Използването на тетрадки улеснява интерактивната разработка или изследване (с Raku)

- Възможност за визуализиране и изчертаване на резултати (получени с Raku)

- Комбиниране на оценките с други езици за програмиране
...
Ето пример за използване на сериализиращата команда `encode-to-wl` за конвертиране на табличен набор от данни, генериран в Raku
с командата `random-tabular-dataset` и извеждането му в Mathematica като "native"
...
WL е езикът за програмиране в софтуерната система." Или нещо подобно.

------

## Твърде зелено, за да бъде червено

... *или* ***"Не имплементирах преобразованив данни чрез Red щото не можах да го инсталирам"***.

В този документ използвам стандартни структури от данни за Raku. Щеше да е хубаво да покажа примери за
на данни с помощта на Raku пакета
...
Разбира се, като създаваме и улесняваме самозваните учени в областта на данните и баристите на кодове, 
ние разпределяме по-равномерен начин бъдещето, което е вече пристигнало.

Ето някои от моите бъдещи планове за събиране на данни и обработка на данни с помощта на Raku
които ще разпределят пристигналото бъдеще още по-равномерно:

- Имплементации на функционалности за събиране на данни.

- Имплементация на функциите за събиране на данни на разговарящ агент.

    - Реализирах такъв агент в WL, [AAv5].

    - Би било и хубаво, и интересно да се направи реализация в Raku.

- По-обширни тестове на единици.

    - Досега тестовете за единици са на минимално ниво.
...
- Преводи на команди на естествен език за система(и) за обектно-релационно картографиране като `Red`, [FCO1].

- Конвертиране на единичните тестове за обработка на данни на Mathematica в тестове на Raku.

    - Това се отнася както за генерирането на код от тестове на команди на естествен език, така и за тестовете на операции за обработка на данни.

**Забележка:** Цитатът, перифразиран по-горе, и
[приписва се на Уилям Гибсън](https://quoteinvestigator.com/2012/01/24/future-has-arrived/)
е:
