...
# Въведение в обработка на данни с Raku
...
Искам да отбележа, че намирам термина "изкуствен интелект" (ИИ) за брилянтна маркетингова фраза за извличане на пари от
военния комплекс на САЩ и всякакви финансови инвеститори. 
Когато хора, които твърдят, че са, да речем, "професионалисти в областта на науката за данните"
...
Но нека да се върнем към ускоряването на идването на следващота зима на ИИ. Ето го моят план:
...
Печалбата в точка 5 идва от това, че "полето" е "изчистено", следователно по-малка конкуренция за получаване на инвестиции и финансиране.
...
добре познати таблици (или масиви) от данни, и за генериране на случайни таблици от данни.

Този документ е доста техничен - читателите могат просто да бегло прочетат следващата секция, да прочетат секцията
"Правим го като Cro" и да приключат. Някои може да искат да погледнат и да прескочат супертехническата версия
...
**Забележка:** Целевата аудитория на този документ се състои предимно от хора, запознати с културите и култовете на Perl (Пърл) и Raku (Раку).
...
Преводът на някои понятия и фрази от английски на български е, може би, спорен. (И напълно раздразнителен за някои читатели.) 
Поради тази причина прилагам речник на технически думи и фрази.
...
(ii) възможности за генериране на код на Raku за обработка на данни, бих казал, че тези дрехи могат да бъдат подходящо облечени.
...
3. Добър "потребителският опит" може да се постигне или осигури чрез други парадигми и пакети за преобразуване на данни.
...
Редът в по-горният списък показва важността по време на имплементирането на съответните функционалности:
...
Целевите потребители са даннови учени (на пълно работно време, на непълно работно време, или напълно начинаещи), които искат:
...
При обработката и анализа на данни следните три операции не са основни, но все пак са фундаментални:
...
[AAp3], предоставя функции за получаване на (сравнително добре познати) даннови масиви.
...
Ето хистограма на разпределението на броя на редовете в примерните данновите масиви
...
**Забележка:** Думата "wolfram" може да бъде разбита като "wolf" (вълк) и "ram" (овен).
...
[Raku-to-WL сериялизатор](https://github.com/antononcube/Raku-Mathematica-Serializer),
...
... *или* ***"Не имплементирах преобразования данни чрез пакета Red щото не можах да го инсталирам"***.

В този документ използвам стандартни структури от данни на Raku. Щеше да е хубаво да покажа примери за
...
ние разпределяме по по-равномерен начин бъдещето, което е вече пристигнало.

Ето някои от моите бъдещи планове за събиране и обработка на данни с помощта на Raku,
